...
// Date (Time interval since reference date) as string.
...
/// Reading as string for the main U.I.
static NSString *LegendString(CGFloat temp, CGFloat co2) {
...
/// Reading as string for the dock icon.
static NSString *IconLegendString(CGFloat temp, CGFloat co2) {
  NSString *tempS = temp <= 0 ? @" - " : [NSString stringWithFormat:@"%3.1f°", temp];
  NSString *co2S = co2 <= 0 ?  @" - " : [NSString stringWithFormat:@"%3.0f", co2];
...
@interface CO2Manager() <MouseDownProtocol>
...
@implementation CO2Manager

- (void)setup {
  self.hardwareQueue = [self constructWorkQueueWithQuality:NSQualityOfServiceUtility];
  if (0.0 == self.warningThreshold) {
    _warningThreshold = 800;
  }
  if (0.0 == self.dangerThreshold) {
    _dangerThreshold = 1200;
  }
  ...
  self.label.stringValue = LegendString(self.reading.temp, self.reading.co2);
  self.contentView.window.nextResponder = self;
...
- (void)setDangerThreshold:(CGFloat)dangerThreshold {
  if (_dangerThreshold != dangerThreshold) {
...
/// Return an NSAttributedString containing a table of min and max values, with times, as attributed tab separated values
...
/// Update the U.I. to show the new Reading.
- (void)update {
...
/// Update the U.I. background color to mirror the yellow or red LED on the monitor.
- (void)updateBackgroundColor {
  struct CGColor *color = [[NSColor systemBlueColor] CGColor];
  if (self.dangerThreshold < self.reading.co2) {
    color = [[NSColor systemRedColor] CGColor];
  } else if (self.warningThreshold < self.reading.co2) {
    color = [[NSColor systemYellowColor] CGColor];
  }
  self.contentView.layer.backgroundColor = color;
}

/// Draw the Reading on the dock tile.
- (void)drawDockImage {
  NSImage *iconImage = [[NSImage imageNamed:@"iconBlank.png"] copy];
  [iconImage lockFocus];
  CGRect frame = NSMakeRect(3, 3, 118, 95);
  CGRect bounds = frame;
  self.iconLabel.stringValue = IconLegendString(self.reading.temp, self.reading.co2);
